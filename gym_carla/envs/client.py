				...
				if isinstance(self._control, carla.VehicleControl):
					if event.key == K_k:
						...
						world.hud.notification('%s Transmission' %
											   ('Autonomous' if self._autonomous_enabled else 'Non-Autonomous'))
					if not self._autonomous_enabled:
						if event.key == K_q: #toggle reverse gear
							self._control.gear = 1 if self._control.reverse else -1
						elif event.key == K_m: #toggle manual/automatic transmission
							self._control.manual_gear_shift = not self._control.manual_gear_shift
							self._control.gear = world.player.get_control().gear
							world.hud.notification('%s Transmission' %
												   ('Manual' if self._control.manual_gear_shift else 'Automatic'))
						elif self._control.manual_gear_shift and event.key == K_COMMA: #gear up
							self._control.gear = max(-1, self._control.gear - 1)
						elif self._control.manual_gear_shift and event.key == K_PERIOD: #gear down
							self._control.gear = self._control.gear + 1
						elif event.key == K_p and not (pygame.key.get_mods() & KMOD_CTRL): #toggle autopilot
